 ...
 * and creates {@link SourcePartition}s for each group.
 *
...
public abstract class DataLakeConnector implements Connector {
     ...
     *   <li>Creates a {@link SourcePartition} for each group</li>
     * </ol>
     */
    @Override
    public List<SourcePartition> planPartitions(ConnectorPlan plan, DistributionHints hints) {
     ...
     *
     * <p>Default implementation creates a {@link SourcePartition} with the plan
     * and aggregated size estimates from the file tasks.
     *
     * @param plan The original plan
     * @param tasks The file tasks for this partition
     * @return A SourcePartition ready for execution
     */
    protected SourcePartition createPartition(DataLakePlan plan, List<FileTask> tasks) {
        long totalBytes = tasks.stream().mapToLong(t -> t.estimatedBytes().orElse(0L)).sum();
        long totalRows = tasks.stream().mapToLong(t -> t.estimatedRows().orElse(0L)).sum();
        return new SourcePartition(plan, null, OptionalLong.of(totalRows), OptionalLong.of(totalBytes));
    }
