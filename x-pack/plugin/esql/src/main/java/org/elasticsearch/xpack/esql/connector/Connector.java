...
public interface Connector {
     ...
     *
     * <p><b>Expression interpretation:</b> The {@link UnresolvedSource#expression()} is
     ...
     */
    ConnectorPlan resolve(UnresolvedSource source, ResolutionContext context);
    ...
    default PhysicalPlan createPhysicalPlan(ConnectorPlan plan, PhysicalPlanningContext context) {
        return new ConnectorExec(((LeafPlan) plan).source(), plan);
    }
     ...
     */
    default List<SourcePartition> planPartitions(ConnectorPlan plan, DistributionHints hints) {
        // Default: single partition (coordinator-only execution)
        return List.of(SourcePartition.single(plan));
    }
     ...
     * @param context Execution context (DriverContext, BlockFactory, etc.)
     * @return Factory that creates source operators for each driver
     */
    SourceOperatorFactory createSourceOperator(SourcePartition partition, ExecutionContext context);
    ...
    interface ExecutionContext {
        // Placeholder for DriverContext, BlockFactory, etc.
    }

    /**
     * Factory for creating source operators.
     */
    interface SourceOperatorFactory {
        /**
         * Create a source operator for the given driver.
         */
        Object get(Object driverContext);
